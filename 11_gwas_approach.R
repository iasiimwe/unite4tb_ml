...
n_true_snps <- 9
topx <- 100 # To get the top 100 SNPs as these will be used for prescreening
  ...
  for (n_snp in n_snps) {
    ...
    for (j in 1:n_datasets) {
      ...
      system(paste0("/pub59/iasiimwe/plink2 --bfile ", dat_path, " --pheno ", 
                    paste0("pheno_", n_snp, ".txt --glm dominant --out "),  paste0(effect_scenario, "/gwas/gwas_",n_snp, "_", j)))

      # Read the results
      results <- as_tibble(fread(paste0(effect_scenario, "/gwas/gwas_",n_snp, "_", j, ".eta_TVCl.glm.linear"))) 
      ...
      method_list$Cl[j] <- paste0(results$SNP[1:topx], collapse = "; ")
      time_list$start[j] <- paste("T", as.character(ymd_hms(start)))
      time_list$end[j] <- paste("T", as.character(ymd_hms(end)))
      
      # Save nominally significant results
      write.csv(results[results$P < 1e-5,],  
                file = paste0(effect_scenario, "/gwas/gwas_", n_snp, "_", j, ".csv"), 
                row.names = FALSE)
      ...
      write.csv(results,  
                file = paste0(effect_scenario, "/gwas/plot/gwas_",n_snp, "_", j, ".csv"), 
                row.names = FALSE)
      message(paste0(round(j*100/n_datasets, 3), "% complete"))
    }
    write.csv(time_list, paste0(folder_path, "/", n_snp, "_time_gwas.csv"), row.names = FALSE)
    write.csv(method_list, paste0(folder_path, "/", n_snp, "_results_gwas.csv"), row.names = FALSE) 
  }
}
