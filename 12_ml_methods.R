...
n_snps <- c("10_4", "10_5", "10_6") # "10_5", "10_6" "Error: protect(): protection stack overflow" Memory issues with higher SNPs, even with glmnet
    ...
    for (j in 1:n_datasets) {
      ...
      if (n_snp %in% c("10_3", "10_4")) {
        ...
        tibble(old = file) %>%
          ...
          pull(new) %>%
          write.table(file = paste0("SNPs_to_extract_", n_snp, ".txt"), quote = FALSE, row.names = FALSE, col.names = FALSE)
        
        # Get new dataset
        dat_path <- paste0(n_snp, "/dat_", n_snp, "_", j)
        temp_path <- paste0("new_temp_", n_snp)
        system(paste0("/pub59/iasiimwe/plink1.9/plink --bfile ", dat_path,
                      " --extract SNPs_to_extract_", n_snp, ".txt --recode tab --out ", temp_path))
        ped <- tibble(fread(paste0(temp_path, ".ped"), header = FALSE))
        map <- tibble(fread(paste0(temp_path, ".map"), header = FALSE))
        snps <- pull(map, V2)
        ped_start <- ped %>% select(V1:V6) # This selects FID, IID, father's ID, mother's ID, sex and phenotype data
      ...
      colnames(ped_snps)[1:n_true_snps] <- paste0("SNP", 1:n_true_snps)
      ped_snps <- ped_snps %>%
        left_join(etas) %>%
        relocate(y, .before = 1) %>%
        select(-ID2) 
      ...
      for (k in seq_along(ml_methods)) {
        ...
        if (ml_methods[[k]] %in% c("glmnet", "rf")) {
          if (n_snp %in% c("10_5", "10_6")) next
          top_covariate_tibble <- get_top_covar_tb_ml(study, ml_methods[[k]], j, set_seed)
          ...
          top_covariate_tibble <- get_top_covar_tb_ml(study %>% select(y, any_of(topx_snps)), "glmnet", j, set_seed)
          ...
          top_covariate_tibble <- get_top_covar_tb_ml(study %>% select(y, any_of(topx_snps)), "rf", j, set_seed)
        } else {
          stop("Unknown method!")
        }
