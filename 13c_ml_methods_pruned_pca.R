...
for (effect_scenario in scenarios) {
  ...
  for (n_snp in n_snps) {
    ...
    for (j in 1:n_datasets) {
      ...
      temp_path <- paste0("pca_temp_", n_snp)
      ...
      system(paste0("/pub59/iasiimwe/plink1.9/plink --bfile ", dat_patho, 
                    ...
                    cor_threshold1, " --out pca_tagged_snps_", n_snp))
      cor_tb1 <- tibble(fread(paste0("pca_tagged_snps_", n_snp, ".ld"))) %>%
      ...
      for (k in seq_along(ml_methods)) {
        ...
        # Perform PCA
        pca_result <- prcomp(select(study, -y), center = TRUE, scale. = TRUE)
        analysis_tb <- tibble(as.data.frame(pca_result$x)) %>%
          mutate(y = study$y, .before = 1)
        
        # Find the top-m contributing SNPs
        top_SNPs <- tibble(PC = colnames(pca_result$rotation),
                           top_SNPs = "")
        for (PC in seq_along(colnames(pca_result$rotation))) {
          pc_loadings <- rev(sort(abs(pca_result$rotation[, PC])))[1:n_true_snps]
        ...
        if (length(SNPs_to_regress) > n_true_snps) {
          ...
          tibble(new = SNPs_to_regress) %>% 
            ...
            select(old) %>%
            write.table(file = paste0("pca_SNPs_to_extract_", n_snp, ".txt"), quote = FALSE, row.names = FALSE, col.names = FALSE)
          
          # Get new dataset
          temp_path <- paste0("pca_new_temp_", n_snp)
          system(paste0("/pub59/iasiimwe/plink1.9/plink --bfile ", dat_patho,
                        " --extract pca_SNPs_to_extract_", n_snp, ".txt --recode tab --out ",
                        temp_path))
          ...
          SNPs_to_regress <- top_covariate_tibble$Covariates
          SNPs_to_regress <- gsub("`", "", SNPs_to_regress)
        }
        
        # Link to the pruned data, split the comma-separated SNPs into a vector (doesn't work for 10_6, as number of SNPs is quite high)
        SNPs_to_regress <- tibble(Covariates = SNPs_to_regress) %>% 
          left_join(cor_tb1) %>%
    ...
    for (k in seq_along(ml_methods)) {
      write.csv(time_list[[k]], paste0(folder_path, "/pca_", n_snp, "_", ml_methods[[k]], "_time.csv"), row.names = FALSE)
      write.csv(method_list[[k]], paste0(folder_path, "/pca_", n_snp, "_", ml_methods[[k]], "_sim_results.csv"), row.names = FALSE)
    } 
  }
}
