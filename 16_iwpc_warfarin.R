...
# -------------------------------
# cd /pub59/iasiimwe/TB/iwpc; source /pub59/iasiimwe/miniconda3/bin/activate base
# zcat /pub59/iasiimwe/IWPC/iwpc_imputed/chr10_qc_bgzip.vcf.gz | head -18
...
#  --vcf /pub59/iasiimwe/IWPC/iwpc_imputed/chr${i}_qc_bgzip.vcf.gz \
...
#  --bfile /pub59/iasiimwe/IWPC/iwpc_genotyped/geno_QC \
#  --chr ${i} --from-bp ${bp1} --to-bp ${bp2} \
#  --geno 0.05 --maf 0.01 --hwe 0.000001 \
#  --recode tab --out original_iwpc_chr_${i}
# done
...
# --gzvcf /pub59/iasiimwe/IWPC/iwpc_imputed/chr${i}_qc_bgzip.vcf.gz \
# --out iwpc_chr_${i} \
...
for (i in c(10, 16)) {
  ...
  bim <- fread(paste0("iwpc_chr_", i, ".bim")) %>%
    select(SNP = V2, POS = V4)
  
  # Get info scores
  info <- fread(paste0("/pub59/iasiimwe/IWPC/iwpc_imputed/chr", i, ".info.gz")) %>%
  ...
  imp_dat <- fread(paste0("iwpc_chr_", i, ".GP.FORMAT")) %>%
    select(-CHROM) %>%
    filter(POS %in% bim$POS)
  fwrite(imp_dat, paste0("iwpc_chr_", i, ".GP.txt"))
}
...
for (chr in chrs) {
  ...
  ped <- tibble(fread(paste0("original_iwpc_chr_", chr, ".ped"), header = FALSE))
  map <- tibble(fread(paste0("original_iwpc_chr_", chr, ".map"), header = FALSE))
  snps <- pull(map, V2)
  ...
  imp_dat <- fread(paste0("iwpc_chr_", chr, ".GP.txt")) 
...
# Get covariates and phenotypes (R) # https://pubmed.ncbi.nlm.nih.gov/39163621/
# ---------------------------------
library(tidyverse)
library(data.table)

dat <- fread("/pub59/iasiimwe/IWPC/gwas/iwpc.sample") 
...
covs <- dat %>%
  ...
  select(FID, IID, age = Age_years, sex = GenderMale, weight = Weight_kg, target_inr = Target_INRThree, 
         sim_amio = Simvastatin_amiodaroneYes, C1:C2)
write.table(covs, "iwpc_covariates.txt", sep = " ", row.names = FALSE, quote = FALSE)
...
covs <- dat %>%
  ...
  select(FID, IID, age = Age_years, sex = GenderMale, weight = Weight_kg, target_inr = Target_INRThree, 
         sim_amio = Simvastatin_amiodaroneYes, C1:C2)
write.table(covs, "iwpc_covariates_imputed.txt", sep = " ", row.names = FALSE, quote = FALSE)

# Pheno
pheno <- dat %>%
  mutate(FID = 0, IID = paste0(ID_1, "_", ID_2)) %>%
  select(FID, IID, Ln_dose_mg_week)
write.table(pheno, "iwpc_phenotypes.txt", sep = " ", row.names = FALSE, quote = FALSE)

# Pheno imputed
pheno <- dat %>%
  mutate(FID = paste0(ID_1, "_", ID_2), IID = FID) %>%
  select(FID, IID, Ln_dose_mg_week)
write.table(pheno, "iwpc_phenotypes_imputed.txt", sep = " ", row.names = FALSE, quote = FALSE)


# Perform traditional GWAS
# ------------------------
# # Conduct QTL analysis
# for outcome in Ln_dose_mg_week; do
# for i in 10 16; do
#  /pub59/iasiimwe/plink2 \
#   --bfile iwpc_chr_${i} \
#   --pheno iwpc_phenotypes.txt --pheno-name ${outcome} \
#   --covar iwpc_covariates.txt --covar-variance-standardize \
#   --glm \
#   --out iwpc_chr_${i}
# done
# done
...
#   --pheno iwpc_phenotypes_imputed.txt --pheno-name ${outcome} \
#   --covar iwpc_covariates_imputed.txt --covar-variance-standardize \
#   --glm \
#   --out imputed_datasets/iwpc_chr_${chr}_dat_${i}
# done
# done
# done
...
for (outcome in outcomes) {
  ...
  for (chr in chrs) {
   ...
   dat <- fread(paste0("iwpc_chr_", chr, ".", outcome, ".glm.linear")) %>%
     filter(TEST == "ADD") %>%
...
write.table(dat_all_outcome, "Traditional_manhattan_iwpc.txt", sep = " ", row.names = FALSE, quote = FALSE)
...
for (outcome in outcomes) {
  ...
  for (chr in chrs) {
    ...
    for (dataset in datasets) {
      dat <- fread(paste0("imputed_datasets/iwpc_chr_", chr, "_dat_", dataset, ".", outcome, ".glm.linear")) %>%
        filter(TEST == "ADD") %>%
...
write.table(dat_all_outcome, paste0("Imputed_", length(datasets), "_datasets_manhattan_iwpc.txt"), sep = " ", row.names = FALSE, quote = FALSE)
...
# Read data
file_names <- c("Traditional_manhattan_iwpc.txt",
                "Imputed_10_datasets_manhattan_iwpc.txt") # Focus on 10 as no additional benefit with 100
...
pl <- ggplot(dat, aes(x = BP_cum, y = -log10(P))) +
  ...
  coord_cartesian(ylim = c(0, -log10(1.76855e-16)))
x <- 0.7
graph2ppt(pl, "Fig_3c_iwpc", append = FALSE, width = 5.5 * x, height = 10 * x)
...
for (outcome in outcomes) {
  ...
  for (chr in chrs) {
    ...
    # Covariates
    covs <- fread("iwpc_covariates.txt") %>%
      rename(ID = IID) %>%
      select(-FID)
    
    # Phenotype
    pheno <- tibble(fread("iwpc_phenotypes.txt"))[, c("IID", outcome)]
...
write.table(dat_all_outcome, "iwpc_ML_methods.txt", sep = " ", row.names = FALSE, quote = FALSE)
...
pl <- ggplot(dat, aes(x = BP_cum, y = -log10(P))) +
  ...
  coord_cartesian(ylim = c(0, -log10(1.76855e-16)))
x <- 0.7
graph2ppt(pl, "Fig_6b_iwpc", append = FALSE, width = 5.5 * x, height = 10 * x)
