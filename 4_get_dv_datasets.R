...
initializeLixoftConnectors(software = "simulx", path = "/pub59/iasiimwe/Lixoft/MonolixSuite2023R1/", force = TRUE)
library(tidyverse)
library(data.table)
library(ggh4x)
...
for (effect_scenario in c("high", "low")) {
  ...
  if (!dir.exists(paste0("sim_DV_", effect_scenario))) dir.create(paste0("sim_DV_", effect_scenario))
  for (j in 1:n_datasets) {
    # Convert to ped format
    system(paste0("/pub59/iasiimwe/plink1.9/plink --bfile 10_3/dat_10_3_", j, " --recode tab --out temp"))
    ...
    # Bring the true SNPs to the first columns and change to 'dominant' coding
    ped_snps <- ped_snps %>%
      select(any_of(true_snps)) %>%
    ...
    write.csv(sim_results, paste0("sim_DV_", effect_scenario, "/sim_data", j, "_", sim_date, ".csv"), row.names = FALSE, quote = FALSE)
    
    message(paste0("##################\n##################\n##################\nEffect Scenario: ", 
                   effect_scenario, ", ", round(j * 100/n_datasets, 2), 
                   "% complete\n##################\n##################\n##################"))
  }
}
...
nonmem_tb <- tibble()
for (j in 1:4) nonmem_tb <- bind_rows(nonmem_tb, mutate(read_csv(paste0("sim_DV_high/sim_data", j, "_", sim_date, ".csv"), show_col_types = FALSE), Simulation = paste0("Dataset ", j)))
...
x <- 0.8
ggsave("simulated_dv_snps_high.png", height = 9 * x, width = 16 * x)


# Check the first four datasets
nonmem_tb <- tibble()
for (j in 1:4) nonmem_tb <- bind_rows(nonmem_tb, mutate(read_csv(paste0("sim_DV_low/sim_data", j, "_", sim_date, ".csv"), show_col_types = FALSE), Simulation = paste0("Dataset ", j)))
...
nonmem_tb %>%
  ...
  theme(legend.position = "bottom",
        ...
        plot.title = element_text(hjust = 0, vjust = 0, size = 14, face = "bold"))
x <- 0.8
ggsave("simulated_dv_snps_low.png", height = 9 * x, width = 16 * x)
