...
for (effect_scenario in c("high", "low")) {
  ...
  message(paste0("Starting complete data analysis for Effect Scenario: ", effect_scenario, "!"))
  
  # Add folders if they don't exist
  folder_path <- paste0("missingness_", effect_scenario)
  ...
  write.csv(results_matrix, paste0(folder_path, "/complete.csv"), row.names = FALSE, quote = FALSE)
  write.csv(time_list[[k]], paste0(folder_path, "/", ml_methods[[k]], "_time.csv"), row.names = FALSE)
  
  # Obtain plots
  # ------------
  # Reshape the data for plotting
  dat <- read_csv(paste0(folder_path, "/complete.csv"), show_col_types = FALSE)
  ...
  x <- 0.7
  ggsave(paste0(folder_path, "/true_estimates_", effect_scenario, ".png"), height = 12 * x, width = 9 * x)
