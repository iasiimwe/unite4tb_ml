...
for (mechanism in mechanisms) {
  for (missing_percentage in missing_percentages) {
    message(paste0("Mechanism: ", mechanism, "\n     missing %: ", missing_percentage))
    
    folder_path <- paste0("michigan/", mechanism, "_", missing_percentage)
    ...
    for (k in 1:n_datasets) {
      ...
      geno_path <- paste0("missing/", missing_percentage, "/dat_", mechanism, "_", k)
      ...
      tibble(fread("true_snps_lifted.bim", header = FALSE, sep = "\t")) %>%
        ...
        write.table(file = paste0("true_covar/true_snps_", mechanism, "_", missing_percentage, "_", k, "_GRCh37.txt"), quote = FALSE, row.names = FALSE, col.names = TRUE)
      
      # QC process, Step 2: Convert to ped/map - do not exclude the true_snps from the genetic data
      system(paste0("/pub59/iasiimwe/plink1.9/plink --bfile ", geno_path, " --recode --out temp_michigan"))
      ...
      system(paste0("/pub59/iasiimwe/plink1.9/plink --bfile temp_michigan-updated-chr1 --recode vcf --out ", folder_path, "/chr", k))
      system(paste0("/pub59/iasiimwe/htslib/htslib-1.3.2/bgzip ", folder_path, "/chr", k, ".vcf"))
    ...
    write.csv(time_tb, paste0(folder_path, "/time.csv"), row.names = FALSE)
  }
}
