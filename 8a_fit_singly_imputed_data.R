...
imputation_methods <- c("CCA", "Mode", "michigan_afr")
  ...
  for (k in seq_along(imputation_methods)) {
    ...
    for (mechanism in mechanisms) {
      for (missing_percentage in missing_percentages) {
        if (all(imputation_method %in% c("michigan_afr") & mechanism %in% c("MAR", "MNAR"))) next
        if (all(imputation_method %in% c("michigan_afr") & missing_percentages %in% c(10, 20, 50))) next
        ...
        if (imputation_method == "michigan_afr") path_to_save <- paste0(path_to_save, "_single")
