...
dominant_coding_fn <- function(x, r2, y) {  # y is percent missingness obtained from the original dataset
  ...
  # If proportion missing == 0, uncertainty is zero while if proportion missing is 1, uncertainty is at the highest
  # For genotyped samples, uncertainty will also be added based on the missingness proportion and R2
...
generate_imputed_datasets <- function(df, r2_df, n = 5, ymissing, seed = 7) {
  set.seed(seed)
  ...
  for (i in 1:n) {
    ...
    for (col in paste0("SNP", 1:9)) {
      y <- ymissing[[col]]
      imputed_dataset[[col]] <- dominant_coding_fn(x = df[[col]], r2 = pull(filter(r2_df, SNP == col)), y) # , n, maf
    }
...
# Function to replace 'imputed' values with non-missing values in a single column
replace_non_na <- function(col_name, df1, df2) df1[[col_name]] <- ifelse(is.na(df2[[col_name]]), df1[[col_name]], df2[[col_name]])
...
imputation_methods <- c("michigan_afr")
mechanisms <- c("MCAR", "MAR", "MNAR")
missing_percentages <- c(5, 10)
effect_scenarios <- c("high", "low")

for (effect_scenario in effect_scenarios) {
    ...
    for (mechanism in mechanisms) {
      for (missing_percentage in missing_percentages) {
        ...
        for (j in 1:n_datasets) {
          ...
          # Get original missing dataset (to identify the ones not to impute)
          dat_path2 <- paste0("missing/", missing_percentage, "/dat_", mechanism, "_", j)
          system(paste0("/pub59/iasiimwe/plink1.9/plink --bfile ", dat_path2, " --recode tab --out temp"))
          ...
          colnames(ped_snps2) <- snps 
          
          true_snps2 <- fread(paste0("true_covar/true_snps_", j, ".txt"), header = FALSE, sep = " ")$V1
          ped_snps2 <- ped_snps2 %>%
            select(any_of(true_snps2)) %>% # This allows the order to be the same as in 'true_snps'
            mutate_all(dominant_coding_fn2) %>%
            mutate(ID2 = ped_start$V1)
          colnames(ped_snps2) <- c(paste0("SNP", 1:(ncol(ped_snps2) - 1)), "ID2")
          
          # Get percentage tibble
          percent_vctr <- colMeans(is.na(select(ped_snps2, -ID2)))
          
          # Obtain the multiply imputed datasets
          imputed_dat <- generate_imputed_datasets(ped_snps, r2_tb, n = missing_percentage, percent_vctr)
          ...
          for (m in 1:missing_percentage) {
            ...
            # Apply replace_non_na across all SNP columns
            snp_cols <- setdiff(names(ped_snps), "ID2")
            for (col in snp_cols) {
              # Apply to all - similar with MICE
              ped_snps[[col]] <- replace_non_na(col, ped_snps, ped_snps2)
              
              # # Apply when missingness is low e.g. <= 5%
              # if (percent_vctr[[col]] <= 0.05) ped_snps[[col]] <- replace_non_na(col, ped_snps, ped_snps2)
            }
